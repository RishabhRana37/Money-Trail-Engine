...
import { getStats, getAccounts, getAlerts, generateDataset, analyzeDataset, BASE_URL } from '../api';
...
export default function Dashboard({ datasetId, onDatasetGenerated }) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      showToast(`Analysis loaded — ${alertsList.length} alerts found`, "success");
    } catch (err) {
      console.error(err);
  ...
  const handleReloadAnalysis = async () => {
    try {
      ...
      const res = await getAccounts("risk_desc", 0, 50, 0);
  ...
  // Handle fetching fresh accounts when filters/sort change
  useEffect(() => {
    const fetchFreshAccounts = async () => {
      try {
        setLoadingAccounts(true);
        const res = await getAccounts(sort, minRisk, 50, 0);
        setAccounts(res.accounts || []);
        setTotalAccounts(res.total || 0);
      } catch (err) {
        console.error(err);
  ...
  // Handle loading more accounts when offset increases
  useEffect(() => {
    if (offset === 0) return;
    ...
    const fetchMoreAccounts = async () => {
      try {
        setLoadingAccounts(true);
        const res = await getAccounts(sort, minRisk, 50, offset);
        setAccounts(prev => [...prev, ...(res.accounts || [])]);
        setTotalAccounts(res.total || 0);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const handleLoadDemoDataset = async () => {
    try {
      ...
      const genRes = await generateDataset({
        num_accounts: 200,
      ...
      const anaRes = await analyzeDataset(genRes.dataset_id);
      
      setAnalyzingState('');
      onDatasetGenerated(genRes.dataset_id);
      
      const alertCount = anaRes.alerts_generated || 2;
      showToast(`Analysis complete — ${alertCount} alerts found.`, "success");
      fetchDashboardData();
    } catch (err) {
      console.error(err);
      setError(err.message || 'Data fusion pipeline failed.');
      showToast(err.message || 'Data fusion pipeline failed.', 'error');
      setAnalyzingState('');
    }
  };
  ...
  return (
    <div className={`transition-opacity duration-100 ease-in-out ${isMounted ? 'opacity-100' : 'opacity-0'} flex-1 p-6 space-y-6 max-w-7xl mx-auto w-full text-aura-textLight select-none`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-aura-border pb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={reloading}
            className={`px-3 py-2 bg-[#10141D] hover:bg-aura-panelLight border border-aura-border text-xs font-mono font-bold text-white active:scale-95 transition-all flex items-center justify-center gap-1.5 ${reloading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {reloading ? (
              <>
                <svg className="animate-spin h-3.5 w-3.5 text-aura-accent" fill="none" viewBox="0 0 24 24">
              ...
              <>
                <span className="text-aura-accent text-sm">⟳</span>
                RELOAD ANALYSIS
              </>
            )}
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono">
        <div className="p-4 hud-panel bg-aura-panel/80 shadow-lg">
          ...
          <span className="text-2xl font-medium text-white block mt-1.5">
            {loading ? (
              <div className="animate-pulse bg-gray-700 rounded h-8 w-24 mt-1"></div>
            ) : stats ? (
        ...
        <div className="p-4 hud-panel bg-aura-panel/80 shadow-lg">
          ...
          <span className="text-2xl font-medium text-white block mt-1.5">
            {loading ? (
              <div className="animate-pulse bg-gray-700 rounded h-8 w-32 mt-1"></div>
            ) : stats ? (
        ...
        <div className="p-4 hud-panel border-[#E24B4A]/30 bg-[#E24B4A]/5 shadow-lg">
          ...
          <span className="text-2xl font-medium text-red-400 block mt-1.5">
            {loading ? (
              <div className="animate-pulse bg-gray-700 rounded h-8 w-36 mt-1"></div>
            ) : stats ? (
        ...
        <div className="p-4 hud-panel border-[#E24B4A]/30 bg-[#E24B4A]/5 shadow-lg">
          ...
          <span className="text-2xl font-medium text-red-400 block mt-1.5">
            {loading ? (
              <div className="animate-pulse bg-gray-700 rounded h-8 w-16 mt-1"></div>
            ) : stats ? (
              <AnimatedCounter value={stats.high_risk_accounts} formatter={val => val} />
            ) : "-"}
          </span>
        </div>
      </div>
      ...
      {stats && (
        <>
                ...
                return (
                  <div 
                    ...
                    className={`p-3 border text-center cursor-pointer transition-all duration-200 relative ${
                      value > 0 
                        ? 'bg-aura-panelLight/40 border-[#E24B4A]/40 hover:border-[#E24B4A] text-red-400' 
                  ...
                  >
                    ...
                    <span className={`font-mono text-xl font-medium block mt-1.5 ${value > 0 ? 'text-red-400' : 'text-slate-500'}`}>
                      {value}
                    </span>
                  </div>
                );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-6 space-y-3">
              ...
              <div className="divide-y divide-aura-border bg-black/40 border border-aura-border rounded-none overflow-hidden">
                {stats.top_risk_accounts && stats.top_risk_accounts.slice(0, 5).map((acc) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <span className="font-sans text-[13px] font-medium text-white block truncate">{acc.name || acc.account_id}</span>
          ...
          <div className="border-t border-aura-border pt-6 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-white font-mono uppercase tracking-widest">Network Accounts Ledger</h2>
                <p className="text-xs text-aura-textMuted font-mono mt-1">Audit and filter all monitored identities.</p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-xs font-mono">
                  <span className="text-aura-textMuted">Min Risk: {minRisk}</span>
                  <input 
                    type="range" 
                    min="0" 
                    max="100" 
                    ...
                    className="w-32 accent-aura-accent bg-black/40 border border-aura-border h-1 rounded-lg appearance-none cursor-pointer"
                  />
                </div>

                {/* Sort Dropdown */}
                <div className="flex items-center gap-2 text-xs font-mono">
                  <span className="text-aura-textMuted">Sort:</span>
                  <select 
                    value={sort} 
                    onChange={(e) => {
                      setSort(e.target.value);
                    }}
                    className="bg-black/60 border border-aura-border text-white text-xs px-2 py-1 focus:outline-none focus:border-aura-accent"
                  >
            ...
            <div className="divide-y divide-aura-border bg-black/40 border border-aura-border rounded-none overflow-hidden">
              {accounts.map((acc) => {
                ...
                return (
                  <div 
                    key={acc.account_id}
                    onClick={() => navigate(`/account/${acc.account_id}`)}
                    className="p-3 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-aura-panelLight/50 cursor-pointer transition-colors border-b border-aura-border last:border-b-0 gap-4"
                  >
                    {/* Name + Mono ID */}
                    <div className="flex-1 min-w-0">
                      <span className="font-sans text-[13px] font-medium text-white block truncate">{acc.name || acc.account_id}</span>
              ...
              {/* Loading Accounts Skeleton Rows */}
              {loadingAccounts && (
                Array.from({ length: 5 }).map((_, idx) => (
                  <div key={idx} className="p-4 flex items-center justify-between border-b border-aura-border last:border-b-0 animate-pulse">
            ...
            {/* Load More Button */}
            {!loadingAccounts && accounts.length < totalAccounts && (
